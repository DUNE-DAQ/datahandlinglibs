...
BOOST_AUTO_TEST_CASE(datahandlinglibs_SkiplistLatencyBufferModel_write_and_return)
{
  ...
  const auto [written1, result1] = buffer.write_and_return(std::move(frame1)); // NOLINT(performance-move-const-arg)
  ...
  const auto [written2, result2] = buffer.write_and_return(std::move(frame2)); // NOLINT(performance-move-const-arg)
